...
class Host:
    ...
    def to_json(self):
        domains = \
            [] if not self.domains or self.domains[
                0] == 'Domain name not found' else self.domains
        return {
            "ip": str(self.ip),
            "domains": domains,
...
class HostWithScore(Host):
    ...
    def to_csv(self):
        ...
        csv = f"{self.ip};{domains};{self.contacts};{round(self.risk, 20)}"
